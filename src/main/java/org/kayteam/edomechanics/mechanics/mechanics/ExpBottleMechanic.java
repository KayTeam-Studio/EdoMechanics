...
public class ExpBottleMechanic extends Mechanic {

    public ExpBottleMechanic(EdoMechanics plugin, Player player, ItemStack itemUsed) {
        super(plugin, MechanicType.EXP_BOTTLE, player, itemUsed);
    }
    ...
    public void actions() {
        ...
        final Projectile projectile = getPlayer().getWorld().spawn(getPlayer().getEyeLocation().add(
                ...
                getPlugin().getMechanicManager().getProjectileTypes().get(MechanicType.EXP_BOTTLE));
