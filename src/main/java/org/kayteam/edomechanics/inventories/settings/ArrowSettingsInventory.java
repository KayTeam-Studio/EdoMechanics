...
public class ArrowSettingsInventory extends InventoryBuilder {
    ...
    public ArrowSettingsInventory(EdoMechanics plugin) {
        super(plugin.getInventories().getString("settings.arrow.inventoryTitle"), 5);
        Yaml settings = plugin.getSettings();
        Yaml inventories = plugin.getInventories();
        fillItem(() -> inventories.getItemStack("settings.arrow.panel"));
        // Back
        addItem(0, () -> inventories.getItemStack("settings.arrow.back"));
        addLeftAction(0, (player, i) -> plugin.getInventoryManager().openInventory(player, new MechanicListInventory(plugin)));
        // Close
        addItem(8, () -> inventories.getItemStack("settings.arrow.close"));
        addLeftAction(8, (player, i) -> player.closeInventory());
        // Cooldown
        addItem(20, () -> Yaml.replace(inventories.getItemStack("settings.arrow.cooldown"), new String[][] {{"%seconds%", settings.getInt("mechanics.witherSkull.cooldown") + ""}}));
        ...
        // Min Damage
        addItem(21, () -> Yaml.replace(inventories.getItemStack("settings.arrow.minDamage"), new String[][]{{"%damage%", settings.getInt("mechanics.witherSkull.minDamage") + ""}}));
        ...
        // Max Damage
        addItem(22, () -> Yaml.replace(inventories.getItemStack("settings.arrow.maxDamage"), new String[][]{{"%damage%", settings.getInt("mechanics.witherSkull.maxDamage") + ""}}));
        ...
        // Permission to use
        addItem(23, () -> Yaml.replace(inventories.getItemStack("settings.arrow.permissionToUse"), new String[][]{{"%permission%", settings.getString("mechanics.witherSkull.permissionToUse")}}));
        addLeftAction(23, (player, i) -> {
            player.closeInventory();
            Yaml messages = plugin.getMessages();
            messages.sendMessage(player, "mechanics.arrow.inputNewPermissionToUse");
        ...
        addItem(24, () -> Yaml.replace(inventories.getItemStack("settings.arrow.permissionToBypassCooldown"), new String[][]{{"%permission%", settings.getString("mechanics.witherSkull.permissionToBypassCooldown")}}));
        addLeftAction(24, (player, i) -> {
            player.closeInventory();
            Yaml messages = plugin.getMessages();
            messages.sendMessage(player, "mechanics.arrow.inputNewPermissionToBypassCooldown");
            plugin.getInputManager().addInput(player, new ChatInput() {
                @Override
                public boolean onChatInput(Player player, String input) {
